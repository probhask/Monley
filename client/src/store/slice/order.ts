...
import { Order, OrderInitialState, OrderProductFuncParams, UserInitailState } from "../../types";
import toast from "react-hot-toast";

export const orderProduct = createAsyncThunk('orderProduct', async (param: OrderProductFuncParams,{getState}):Promise<Order> => {
    const { user } = getState() as {user:UserInitailState};
    const custId = user.data?.custId;
    
    
    try {
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/order`, {...param,custId })
...
export const getUserOrderList = createAsyncThunk('getUserOrderList', async (_,{getState}) => {
     const { user } = getState() as {user:UserInitailState};
    const custId = user.data?.custId;
    try {
...
const orderSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(getUserOrderList.pending, (state) => {
        ...
        builder.addCase(orderProduct.pending, (state) => {
            ...
            state.isLoading = false;
            state.isError = '';
            toast.success(`Order Success \n Your orderId:${action.payload.orderId}`)
            setTimeout(() => {
